    ...
    <div ref={setNodeRef} className="bg-[#1e293b] shadow-lg border border-white/10 text-white w-64 min-w-[256px] flex-shrink-0 p-3 rounded-xl h-[320px] flex flex-col relative">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="relative">
          <button onClick={() => setShowMenu(!showMenu)} className="px-2 text-gray-400 hover:text-white">⋯</button>

          {showMenu && (
            <div className="absolute right-0 mt-2 bg-[#334155] rounded shadow-lg text-sm w-28 z-10">
              <button onClick={() => { setEditingTitle(true); setShowMenu(false); }} className="block w-full text-left px-3 py-2 hover:bg-gray-600">
                Edit
              </button>

              <button onClick={() => deleteList(list.id)} className="block w-full text-left px-3 py-2 hover:bg-red-500">
                Delete
              </button>
            </div>
          )}
      ...
      {showInput ? (
        <div className="mt-2">
          <input
            ...
            className="w-full p-2 rounded bg-[#334155]"
          />

          <div className="flex gap-2 mt-2">
            <button onClick={handleAdd} className="bg-blue-500 px-3 py-1 rounded text-sm">
              Add
            </button>

            <button onClick={() => setShowInput(false)} className="text-gray-300 text-sm">
              Cancel
            </button>
          </div>
        </div>
      ) : (
        <button onClick={() => setShowInput(true)} className="mt-2 text-sm text-gray-300 hover:text-white">
          + Add a card
        </button>
      )}
    </div>
